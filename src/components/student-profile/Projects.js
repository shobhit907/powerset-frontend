...
export default function Projects() {
    ...
    const [projects, setProjects] = useState([{ title: "abc", start_date: "yyyy-mm-dd",end_date: "yyyy-mm-dd",description:"VFDDFV",domain:"abc" }]);
   ...
   // const [id,setId]=useState(0);
    ...
    const handleSave=()=>{
      ...
      for(var i=0;i<projects.length;i++){
        if(!lettersAndSpaces.test(projects[i].title)){
          setErrorText("Project "+String(i+1)+": Title must only Conatin Letters and Numbers");
          return;
        }
        if(!regEx.test(projects[i].start_date) && projects[i].start_date!=""){
          setErrorText("Project "+String(i+1)+": Start Date Must follow yyyy-mm-dd format");
          return;
        }
    ...
    const handleAddClick = (field) => {
         switch(field){
             case 1:
                setProjects([...projects, { title: "", start_date: "yyyy-mm-dd",end_date: "yyyy-mm-dd",description:"",domain:"abc" }]);
